...
class AirCanvas {
  ...
  private setupButtonListeners(): void {
    ...
    shapeButtons.forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (shape) {
          this.objectManager.createFromPrimitive(shape, this.currentColor);
          this.showStatus(`Spawning ${shape}...`, 800);
        }
      });
    });
  ...
  private async init(): Promise<void> {
    try {
      // Start hand tracking
      await this.handTracker.start((landmarks, secondHand) => this.onHandResults(landmarks, secondHand));
  ...
  private onHandResults(landmarks: HandLandmarks | null, secondHand?: HandLandmarks | null): void {
    ...
    // --- TWO-HAND ZOOM ---
    if (landmarks && secondHand) {
      this.isTwoHandMode = true;
      const idx1 = landmarks.landmarks[8]; // index tip of hand 1
      const idx2 = secondHand.landmarks[8]; // index tip of hand 2
      ...
      if (this.lastTwoHandDist !== null) {
        ...
        // Positive delta = hands spreading = zoom in, negative = zoom out
        const zoomFactor = delta * 0.005;
        this.scene3D.zoomCamera(-zoomFactor); // negative because zoomCamera() expects scroll-direction
      }
      this.lastTwoHandDist = dist;
      return; // Don't process single-hand gestures when in two-hand mode
    } else {
      this.isTwoHandMode = false;
  ...
  private handleSwipe(position: { x: number; y: number }): void {
    // Guard: never swipe-delete while holding an object
    if (this.grabbedObject) return;
    // Check if swiping on an object
    const hitObject = this.objectManager.getObjectAtPosition(position.x, position.y);
    if (hitObject) {
      if (hitObject.groupId) {
        // Disassemble the group!
        this.showStatus('Disassembling group...', 1000);
        // Break the group links for all objects in this group
        const groupMembers = this.objectManager.getObjects().filter(o => o.groupId === hitObject.groupId);
        for (const member of groupMembers) {
          member.groupId = null;
          member.linkedObjects = [];

          // Add a little outward explosion force
          ...
          member.targetPosition.add(pushDir);
        }
      } else {
