...
export class GestureDetector {
  ...
  private swipeCooldown: number = 600; // ms cooldown between swipes
  ...
  private detectGestureType(landmarks: HandLandmarks, velocity: Point2D): GestureType {
    ...
    // 1. Check pinch FIRST (thumb tip close to index tip) — highest priority for grab
    ...
    // 2. Check for swipe AFTER pinch (fast horizontal movement) with cooldown
    ...
    // 3. Check for fist (all fingers curled)
    if (this.isFist(landmarks)) {
      return 'fist';
    }

    // 4. Check for open palm (all fingers extended)
    ...
    // 5. Check for draw mode (index finger extended)
