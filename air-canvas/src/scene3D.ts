...
export class Scene3D {
  ...
  private setupLighting(): void {
    // Ambient light for soft overall illumination (darker for hologram contrast)
    this.ambientLight = new THREE.AmbientLight(0x0a1526, 0.8);
    this.scene.add(this.ambientLight);

    // Main directional light (Projector light from below)
    this.directionalLight = new THREE.DirectionalLight(0x00f3ff, 1.2);
    ...
    const fillLight = new THREE.DirectionalLight(0x0066ff, 0.6);
    fillLight.position.set(-5, 0, 3);
    this.scene.add(fillLight);

    // Rim light for edge definition (purple accent)
    const rimLight = new THREE.DirectionalLight(0xbc13fe, 0.5);
